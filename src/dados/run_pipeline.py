"""Roda as duas etapas do pipeline sobre um pool de imagens.
...
crop_and_adjust; o comportamento validado deles fica intacto.
    ...
    python src/dados/run_pipeline.py                     # pool/ (padrao)
    python src/dados/run_pipeline.py --pool pool_206     # outro pool
...
def _pool_name():
    if "--pool" in sys.argv:
        return sys.argv[sys.argv.index("--pool") + 1]
    return "pool"


POOL = f"Dataset_YOLO/{_pool_name()}"
POOL_IMAGES = f"{POOL}/images"
POOL_LABELS = f"{POOL}/labels"
...
# recorte em vez de clipa-las, perdendo as caixas de whitening de canto que a
...
def _point_to_stage1():
    ...
    detector.PREVIEW_DIR = f"{POOL}/preview"
...
def _point_to_stage2():
    ...
    cropper.PREVIEW_DIR = f"{POOL}/preview_corte"
    cropper.OUTPUT_IMAGES_DIR = f"{POOL}/images_final"
    cropper.OUTPUT_LABELS_DIR = f"{POOL}/labels_final"
    cropper.MODE = "crop"
